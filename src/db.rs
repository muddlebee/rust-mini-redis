...
use crate::stream_consumer::{Stream, StreamEntry};
...
struct Shared {
    ...
    impl Db {
        ...
        /// xadd implementation
        pub(crate) fn xadd(&self, name: String, entries: Vec<String>) {
            let mut state = self.shared.state.lock().unwrap();
            // Assuming entries are field-value pairs in sequence
            if entries.len() % 2 != 0 {
                //      return Err("Entries must contain an even number of elements".into());
            }

            let mut stream_entry = StreamEntry::new();

            for entry_slice in entries.chunks(2) {
                if let [field, value] = entry_slice {
                    stream_entry.set_field(field, value);
                }
            }

            let stream_entry_formatted = VecDeque::from(vec![stream_entry]);

            state.streams.set_name_and_entries(name, stream_entry_formatted);
